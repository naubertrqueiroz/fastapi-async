...
async def update_usuario(usuario_id: int, usuario: UsuarioSchemaUp, db: AsyncSession = Depends(get_session)):
    async with db as session:
        query = select(UsuarioModel).filter(UsuarioModel.id == usuario_id)
        result = await session.execute(query)
        ...
        if usuario_up:
            ...
            if usuario.user_admin:
                usuario_up.user_admin = usuario.user_admin
            if usuario.senha:
